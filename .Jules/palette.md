...
**Learning:** When rendering lists of actionable items in the UI (e.g., disambiguation candidate buttons), structuring them within semantic `<ul>` and `<li>` tags improves screen reader accessibility by providing structural context and list count, rather than presenting them as a flat stream of unrelated elements.
**Action:** Always structure lists of similar actionable items within semantic `<ul>` and `<li>` tags, and ensure the interactive children stretch to fill the item (e.g., using `w-full`) to maintain layout and hit areas.
