...
## 2024-05-11 - Semantic Forms for IME Composition and Preserving Focus
**Learning:** Manually tracking "Enter" keys via `onKeyDown` listeners on text inputs can prematurely submit forms during IME (Input Method Editor) composition for languages like Japanese or Chinese. Additionally, disabling the primary text input field during application loading states (`disabled={loading}`) forcibly removes the user's keyboard focus, resulting in a disruptive user experience.
**Action:** Wrap chat inputs and their corresponding buttons in semantic `<form>` tags utilizing `onSubmit` to naturally support IME composition. Keep the input field enabled during network requests to preserve focus, and guard concurrent submissions programmatically in the submit handler (`if (loading) return;`).
