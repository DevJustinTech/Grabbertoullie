...
## 2024-05-20 - Semantic Forms and Input Focus
**Learning:** Using semantic `<form>` natively supports IME (Input Method Editor) composition, avoiding premature submissions when users press 'Enter' in languages like Japanese or Chinese. Also, disabling inputs during loading (`disabled={loading}`) forcibly removes keyboard focus, creating a jarring UX.
**Action:** Always wrap chat inputs and buttons in semantic `<form>` tags with an `onSubmit` handler, rather than relying on `onKeyDown` listeners. To prevent concurrent submissions during loading, use programmatical guards (e.g., `if (loading) return;`) instead of disabling the main text input.
