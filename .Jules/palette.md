...
## 2025-03-06 - Chat Interface Accessibility and Break Words

**Learning:** When implementing live chat interfaces, wrapping the message stream in `role="log"` and `aria-live="polite"` ensures screen readers automatically announce incoming messages without interrupting the user. Additionally, wrapping disambiguation/action lists in semantic `<ul>` and `<li>` elements allows screen readers to announce the number of choices correctly. Finally, long un-spaced strings (like raw URLs) in chat bubbles can break flex layouts unless explicitly handled with text-wrapping utilities like `break-words`.

**Action:** Always apply `role="log"` and `aria-live="polite"` to dynamic message containers. Always structure repeated UI choices within semantic list elements (`<ul>`/`<ol>`), ensuring interactive children stretch to fill the item (e.g., using `w-full`). Apply `break-words` to text containers rendering unpredictable user or bot content.
