...
## 2024-06-03 - Chat Screen Reader Announcements and Overflowing Text
**Learning:** Applying `role="log"` and `aria-live="polite"` to a chat message container ensures screen readers automatically announce new incoming messages without requiring the user to manually discover them. Additionally, long un-wrappable strings (like URLs or very long words) can break the message bubble layout, so `break-words` is essential. Finally, interactive lists of options (like disambiguation results) should be structured with `<ul>` and `<li>` for semantic meaning and easier screen reader navigation, with child buttons stretching to full width using `w-full` to maintain hit areas.
**Action:** Always wrap chat message lists in a container with `role="log"` and `aria-live="polite"`. Use `break-words` on user-generated message text containers to prevent layout breaking. Render lists of actionable UI items (like search results) within semantic `<ul>` and `<li>` tags, and use `w-full` on child interactive elements to maintain layout.
