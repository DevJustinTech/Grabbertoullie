...
## 2025-03-06 - Live Chat Region Accessibility

**Learning:** For live chat interfaces, screen readers will not announce newly appended incoming messages automatically without proper semantic roles. Simply rendering new messages to the DOM is insufficient for non-visual users.

**Action:** Always apply `role="log"` and `aria-live="polite"` to the messages container to ensure screen readers announce incoming messages seamlessly as they are added to the list.
