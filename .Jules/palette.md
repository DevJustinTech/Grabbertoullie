...
## 2025-05-19 - Semantic Forms & Retaining Input Focus

**Learning:** Relying on `onKeyDown="Enter"` to handle form submission in inputs breaks native Input Method Editor (IME) composition commonly used in languages like Japanese or Chinese, leading to premature form submissions. Additionally, toggling `disabled={loading}` on the primary input field forcibly drops the user's keyboard focus during loading states, which breaks keyboard navigation flows.

**Action:** Always wrap chat inputs and their corresponding submit buttons in semantic `<form>` tags utilizing `onSubmit`. Leave the input enabled during loading states (so focus isn't lost), and guard against concurrent or duplicate submissions programmatically inside the submit handler (`if (loading) return;`).
