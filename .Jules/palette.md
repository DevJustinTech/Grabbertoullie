...
## 2024-05-14 - Native Form Submission & Focus Management
**Learning:** For chat interfaces, manually tracking 'Enter' keys via `onKeyDown` listeners breaks native Input Method Editor (IME) composition for languages like Japanese/Chinese, leading to premature submissions. Additionally, disabling the primary text input (`disabled={loading}`) during async operations forcibly removes user keyboard focus, degrading accessibility.
**Action:** Always wrap inputs and buttons in semantic `<form>` tags utilizing `onSubmit`. Leave the input enabled during loading states to preserve focus, and guard concurrent submissions programmatically in the submit handler (`if (loading) return;`).
