...
## 2025-03-05 - Native Forms for IME and Focus Retention

**Learning:** Relying on 'Enter' keydowns via `onKeyDown` listeners for chat inputs breaks or submits prematurely when users type in languages that use IME (Input Method Editors, e.g., Japanese, Chinese). Furthermore, disabling the primary input field during loading states forcibly removes the user's keyboard focus, disrupting accessibility.

**Action:** Always wrap chat inputs and submit buttons in a semantic `<form>` tag and use `onSubmit` to naturally support IME composition. Prevent concurrent submissions programmatically (e.g., `if (loading) return;`) instead of disabling the input itself so that keyboard focus is preserved during loading states.
