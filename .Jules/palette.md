...
**Learning:** When rendering lists of actionable items in the UI (e.g., disambiguation candidate buttons), relying on `div` wrappers creates an accessibility failure. A screen reader user navigating by list semantics will not be able to understand the structure of the options.

**Action:** Whenever implementing lists or repeating elements, always structure them within semantic `<ul>` and `<li>` tags to improve screen reader accessibility. Ensure the interactive children stretch to fill the item (e.g., using `w-full`) to maintain layout and hit areas.
