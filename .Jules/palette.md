...
## 2024-05-24 - Handle IME interactions via semantic forms and preserve focus during loading
**Learning:** Native Input Method Editor (IME) compositions (e.g., Japanese, Chinese typing) can cause premature submissions if relying purely on `onKeyDown` ('Enter') listeners. Disabling the input element `disabled={loading}` during async operations forcibly removes keyboard focus, confusing users and screen readers.
**Action:** Always wrap chat inputs and their corresponding buttons in semantic `<form>` tags utilizing `onSubmit`. Leave inputs enabled during loading states to preserve keyboard focus, and guard concurrent submissions programmatically in the submit handler (`if (loading) return;`).
