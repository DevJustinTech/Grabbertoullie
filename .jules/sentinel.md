...
## 2025-02-14 - SSRF Bypass via Unroutable IPs and Missing HTTP Client Hooks
**Vulnerability:** The SSRF protection logic in `backend/services/security.py` only checked if an IP was private, loopback, multicast, reserved, or link-local. This allowed bypasses using unroutable or shared IP ranges like `0.0.0.0` or `100.64.0.1`. Furthermore, several `httpx.AsyncClient` instances across `main.py`, `search.py`, and `llm.py` failed to utilize the SSRF validation hook (`check_url_hook`) entirely.
**Learning:** Checking for specific types of internal IP addresses is an incomplete approach, as IPv4 is filled with edge cases and unroutable subnets that resolve to local interfaces. Default `httpx` clients do not automatically inherit security validations, making it extremely easy to overlook when instantiating new clients across the codebase.
**Prevention:** Always enforce that an IP must be explicitly globally routable (`is_global`) and not unspecified (`not is_unspecified`). For HTTP clients, centralize client instantiation or enforce a strict linting rule to ensure all clients have the necessary SSRF `event_hooks` injected upon creation.
