...
## 2025-02-20 - Webhook Spoofing Vulnerability Fixed
**Vulnerability:** The WhatsApp webhook endpoint (`/webhook`) in `backend/main.py` processed incoming payloads blindly without verifying the `x-hub-signature-256` HMAC signature, allowing potential webhook spoofing.
**Learning:** External webhook endpoints need to authenticate incoming requests. For Meta webhooks, they send an `x-hub-signature-256` header containing the SHA256 signature of the raw request payload computed using the App Secret. This raw payload is critical to calculate the expected signature correctly, rather than parsing JSON first.
**Prevention:** Always implement signature verification on webhook endpoints before processing the payload. Use `await request.body()` to obtain the raw bytes, `hmac.compare_digest` to prevent timing attacks, and make sure the application secret is managed securely via environment variables.
