...
## 2025-05-28 - Missing Webhook Signature Verification in WhatsApp Integration
**Vulnerability:** The `/webhook` POST endpoint accepted incoming JSON payloads representing WhatsApp messages without validating the `X-Hub-Signature-256` header, allowing unauthenticated attackers to spoof events and trigger internal systems.
**Learning:** For third-party webhooks, especially those triggering resource-heavy backend integrations (like LLM interactions and file processing), missing signature validation completely bypasses authentication layers, allowing arbitrary request execution. Reading `request.json()` prior to validating the raw body bytes modifies the payload and prevents signature calculation.
**Prevention:** Always enforce HMAC-SHA256 signature verification on webhook endpoints before processing the payload. Use `await request.body()` for HMAC verification prior to any parsing.
