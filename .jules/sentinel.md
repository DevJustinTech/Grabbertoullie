...
## 2025-05-02 - Webhook Signature Validation with Raw Body Bytes
**Vulnerability:** The WhatsApp webhook handler (`/webhook` POST) verified the source by implicitly trusting the request origin without any cryptographic validation of the payload. An attacker could easily forge WhatsApp messages by sending fake JSON payloads to the endpoint.
**Learning:** Webhook payloads must be authenticated to ensure they originated from the expected service (like Meta/WhatsApp). When using FastAPI, calculating the HMAC-SHA256 signature requires the original raw body bytes (`await request.body()`). Parsing the body first (e.g., via `await request.json()`) alters the formatting and causes signature verification to always fail.
**Prevention:** Always implement `X-Hub-Signature-256` validation for webhooks. Compute the expected signature using the exact raw bytes received (`await request.body()`) before executing `await request.json()`, and use `hmac.compare_digest` to prevent timing attacks.
