...
## 2025-05-28 - SSRF Bypass via synchronous DNS resolution and inadequate IP checks
**Vulnerability:** The SSRF validation function was blocking the asyncio event loop using synchronous `socket.getaddrinfo`, leaving the server vulnerable to DOS. Furthermore, checking for private/loopback using `ip_obj.is_private` and `ip_obj.is_loopback` was not enough to block unroutable or shared-network IPs like `0.0.0.0` or `100.64.0.1`.
**Learning:** Python's `ipaddress` module requires checking both `ip_obj.is_global` (must be true) and `ip_obj.is_unspecified` (must be false) to properly validate globally routable IPs. Using synchronous DNS resolution blocks the entire event loop in FastAPI during slow resolutions.
**Prevention:** Always use `asyncio.get_running_loop().getaddrinfo` for DNS in async contexts and strictly validate `not ip_obj.is_unspecified` and `ip_obj.is_global`.
