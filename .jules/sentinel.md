...
**Vulnerability:** The WhatsApp `/webhook` POST endpoint accepted unauthenticated payloads, allowing anyone to trigger backend processes.
**Learning:** External webhooks from platforms like Meta require strict validation to ensure authenticity, as they often do not include standard authentication headers but instead use signed request bodies.
**Prevention:** Always implement HMAC-SHA256 signature verification (using `X-Hub-Signature-256`) when exposing webhook endpoints to the public internet, and use `hmac.compare_digest` to prevent timing attacks.
