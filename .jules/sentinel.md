...
## 2025-05-15 - [WhatsApp Webhook Signature Bypass]
**Vulnerability:** The `/webhook` endpoint accepted unverified POST requests by relying only on the presence of the `object` field. The `X-Hub-Signature-256` header was completely ignored, allowing attackers to spoof incoming WhatsApp messages.
**Learning:** Webhook validation is critical to prevent injection attacks and server impersonation. Furthermore, computing an HMAC-SHA256 signature against an incoming payload requires using the raw request body bytes before parsing (e.g., `await request.body()`), otherwise subtle formatting shifts during parsing will invalidate the signature. Using `hmac.compare_digest` is also strictly necessary to mitigate timing attacks against the signature comparison.
**Prevention:** Always extract the raw request body via `request.body()`, retrieve the signature from headers, compute the expected HMAC hash using the provided App Secret, and use a constant-time comparison. Fail fast with an HTTP 401 if the signature is missing or incorrect before parsing the JSON body.
