...
## 2024-05-28 - Missing Webhook Signature Verification
**Vulnerability:** The WhatsApp webhook endpoint (`/webhook` POST) was accepting all incoming JSON payloads without verifying their origin. This allowed attackers to spoof WhatsApp API requests and inject arbitrary payloads, bypassing authentication.
**Learning:** External webhook endpoints inherently lack transport-level authentication from the application's perspective. Relying solely on the endpoint's obscurity or expecting incoming data to always be well-formed from trusted providers creates a massive zero-trust gap.
**Prevention:** Always implement robust cryptographic signature verification for all webhook endpoints. Use the raw request bytes (`await request.body()`) to construct the HMAC-SHA256 signature against the shared application secret, and utilize `hmac.compare_digest` to prevent timing attacks during string comparison.
