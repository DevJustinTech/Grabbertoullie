...
## 2024-05-28 - [WhatsApp Webhook Missing Payload Authentication]
**Vulnerability:** The WhatsApp webhook endpoint (`/webhook`) accepted all POST requests without validating their authenticity, allowing attackers to spoof messages from users to the bot.
**Learning:** Webhook endpoints exposed to the internet must authenticate incoming requests. WhatsApp provides an `X-Hub-Signature-256` header which is an HMAC-SHA256 signature of the raw payload using the App Secret. Processing requests without validating this signature enables arbitrary code execution or spoofed data injections within the bot pipeline.
**Prevention:** Always extract the raw request body and calculate its HMAC-SHA256 signature using the provider's shared secret. Use `hmac.compare_digest` to safely compare the expected signature with the provided signature header before processing any webhook payload.
