...
## 2025-05-16 - Missing WhatsApp Webhook Signature Validation
**Vulnerability:** The application's webhook endpoint (`/webhook` POST) was parsing and processing payloads without verifying the `X-Hub-Signature-256` header, allowing an attacker to trivially spoof payloads from Meta/WhatsApp by sending arbitrary JSON.
**Learning:** Webhook endpoints must authenticate incoming requests. Using native parsing like `await request.json()` first alters the payload format. To properly compute the HMAC-SHA256 signature, the raw byte string from `await request.body()` must be used to ensure the generated digest accurately matches the sent signature.
**Prevention:** Implement `hmac.compare_digest(expected, incoming)` for secure timing-attack prevention on webhook integrations. Always compute the hash using the raw HTTP request body bytes before parsing it into JSON.
