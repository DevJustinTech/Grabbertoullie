...
## 2024-05-14 - Webhook Signature Payload Alteration
**Vulnerability:** Missing signature validation on WhatsApp Webhooks allows payload spoofing. When implementing the validation, calling `request.json()` before calculating the HMAC altered the raw bytes, causing validation to fail.
**Learning:** Webhook signature verification in FastAPI requires the exact raw bytes from the request body (`await request.body()`) to match the upstream HMAC calculation. Parsing JSON beforehand breaks the signature.
**Prevention:** Always perform signature validation using `await request.body()` first, then process `await request.json()` down the line to retain correct HMAC comparison while keeping framework-level schema validation.
