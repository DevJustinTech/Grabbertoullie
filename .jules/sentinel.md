...
## 2024-05-18 - WhatsApp Webhook Authentication Bypass Fix
**Vulnerability:** The WhatsApp incoming webhook POST endpoint lacked any mechanism to authenticate the sender, allowing anyone who discovers the webhook URL to spoof incoming WhatsApp messages by crafting arbitrary JSON payloads.
**Learning:** For webhooks expecting an HMAC signature validation (e.g., using `X-Hub-Signature-256`), you must compute the signature on the raw request body bytes (e.g. `await request.body()`). Calling JSON parsing methods before or attempting to re-serialize the payload will slightly modify the bytes and invalidate the cryptographic signature. It's critical to use `hmac.compare_digest` to prevent timing attacks when comparing signatures.
**Prevention:** Ensure new webhook endpoints always include header-based signature verification and enforce it before doing any body processing.
