...
## 2025-02-18 - Fix Webhook Signature Validation By Reading Raw Body Bytes
**Vulnerability:** The WhatsApp webhook endpoint was missing `X-Hub-Signature-256` signature validation entirely, allowing attackers to easily spoof payloads. When attempting to add validation, parsing the body first using `request.json()` modifies the payload, causing the HMAC hash computation to fail against the webhook sender's original signature.
**Learning:** For webhooks requiring cryptographic signature verification (like WhatsApp), the HMAC signature must ALWAYS be computed using the raw, unparsed HTTP request body bytes (`await request.body()`). Calling `.json()` alters whitespace, order, and escaping, leading to a mismatched hash.
**Prevention:** Always read and validate `request.body()` first during webhook authentication before parsing the payload with `request.json()`.
