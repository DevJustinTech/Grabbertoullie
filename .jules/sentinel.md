...
## 2025-05-24 - Unauthenticated Webhook Endpoint
**Vulnerability:** The WhatsApp webhook endpoint (`/webhook` POST) was accepting unauthenticated requests, allowing anyone to spoof payloads and trigger application logic.
**Learning:** External webhook providers (like Meta) require payload verification to ensure the request originated from them. Without this, the application is vulnerable to unauthorized invocation and data manipulation.
**Prevention:** Always implement signature validation for webhooks. For WhatsApp, this involves computing an HMAC-SHA256 signature of the raw request body using the App Secret and comparing it to the `x-hub-signature-256` header via `hmac.compare_digest` to prevent timing attacks.
