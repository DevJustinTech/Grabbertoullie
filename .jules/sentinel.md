...
## 2025-05-25 - WhatsApp Webhook Authentication Bypass
**Vulnerability:** The `/webhook` POST endpoint lacked signature validation, allowing unauthorized parties to spoof messages.
**Learning:** External webhook endpoints need to verify incoming payloads. For WhatsApp, it's critical to calculate the HMAC-SHA256 signature using the raw request body bytes prior to parsing the payload with `json.loads` or `request.json()`, as the parsing process can alter the payload and cause signature verification to incorrectly fail.
**Prevention:** Always validate webhook signatures using the `X-Hub-Signature-256` header against the raw body bytes. Check for the existence of `WHATSAPP_APP_SECRET` and block requests matching invalid signatures with a 403 Forbidden.
