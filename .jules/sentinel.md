...
## 2025-02-14 - Fix SSRF via 0.0.0.0 bypass
**Vulnerability:** The SSRF protection `is_valid_url` checked for local or private IPs using `ipaddress` properties, but failed to block the unspecified/unroutable address `0.0.0.0`. It also failed open by silently passing DNS resolution exceptions (`socket.gaierror`), relying on httpx later.
**Learning:** `0.0.0.0` is treated differently across platforms and may map to local services. Resolving it using `socket.getaddrinfo` might succeed, but the ipaddress validation missed it because it isn't strictly private, loopback, or reserved. Furthermore, using the synchronous `socket.getaddrinfo` inside FastAPI blocked the async event loop.
**Prevention:** Always check `ip_obj.is_unspecified` and explicitly require `ip_obj.is_global`. Fail securely closed on DNS lookup errors. Use `asyncio.get_running_loop().getaddrinfo()` to prevent event loop blocking.
