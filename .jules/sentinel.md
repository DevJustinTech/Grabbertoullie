...
## 2025-05-28 - Fail-Open Webhook Validation and Timing Attack Vulnerability
**Vulnerability:** The WhatsApp webhook verification (`@app.get("/webhook")`) and event handler (`@app.post("/webhook")`) endpoints silently skipped signature validation if `WHATSAPP_APP_SECRET` was unconfigured, effectively bypassing authentication. Additionally, the verification token was checked using a simple string comparison (`token == WHATSAPP_VERIFY_TOKEN`), making it susceptible to timing attacks.
**Learning:** If webhook endpoints conditionally enforce signature validation based on the presence of a secret, missing configuration causes them to "fail open," allowing an attacker to bypass all security measures. Furthermore, comparing cryptographic tokens with `==` allows attackers to infer the expected value byte-by-byte via timing differences.
**Prevention:** Webhooks must always "fail closed" (e.g., return HTTP 500 or 403) if required secrets are unconfigured. Additionally, always use constant-time string comparison methods like `hmac.compare_digest()` for cryptographic secrets and verification tokens to prevent timing attacks.
