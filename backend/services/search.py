...
async def search_open_library(title: str, author: str = "") -> List[Dict[str, Any]]:
    ...
    query = []
    if title:
        query.append(f"title={urllib.parse.quote_plus(title)}")
    if author:
        query.append(f"author={urllib.parse.quote_plus(author)}")
...
async def search_standard_ebooks(title: str) -> List[Dict[str, Any]]:
    ...
    url = f"https://standardebooks.org/ebooks?query={urllib.parse.quote_plus(title)}"
...
async def search_project_gutenberg(title: str, author: str = "") -> List[Dict[str, Any]]:
    ...
    url = f"https://gutendex.com/books?search={urllib.parse.quote(query_str)}"
...
async def search_semantic_scholar(title: str, author: str = "") -> List[Dict[str, Any]]:
    ...
    query_str = " ".join(query_parts)
    url = f"https://api.semanticscholar.org/graph/v1/paper/search?query={urllib.parse.quote(query_str)}&fields=title,authors,year,isOpenAccess,openAccessPdf&limit=5"
