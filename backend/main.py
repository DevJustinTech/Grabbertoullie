...
async def verify_webhook(request: Request):
    if not WHATSAPP_VERIFY_TOKEN:
        raise HTTPException(status_code=500, detail="Server misconfiguration: missing webhook verification token")
...
async def handle_webhook(request: Request):
    if not WHATSAPP_APP_SECRET:
        raise HTTPException(status_code=500, detail="Server misconfiguration: missing app secret")
