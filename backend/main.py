...
async def verify_webhook(request: Request):
    ...
    if mode and token:
        if not WHATSAPP_VERIFY_TOKEN:
            raise HTTPException(status_code=500, detail="Missing verify token configuration")
...
async def handle_webhook(request: Request):
    ...
    if not WHATSAPP_APP_SECRET:
        raise HTTPException(status_code=500, detail="Missing app secret configuration")
