...
// ⚡ Bolt: Memoized message item to prevent re-rendering the entire chat history on every keystroke
        ...
        <div className="flex flex-col gap-2 mt-2">
          {msg.result.candidates.map((candidate, i) => (
            <button
              ...
              onClick={() => {
                const formatSuffix =
                  msg.result?.format && msg.result.format !== "any" ? ` ${msg.result.format}` : "";
...
export default function Home() {
  ...
  // ⚡ Bolt: Memoized callbacks to maintain stable references for the memoized MessageItem
