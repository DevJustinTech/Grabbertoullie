...
interface MessageItemProps {
  ...
  handleDownload: (url: string, source?: string) => void;
}

// ⚡ Bolt: Extracted the message rendering into a memoized component.
// This prevents the entire message list from re-rendering on every keystroke in the input field,
// significantly reducing main thread blocking during active typing.
const MessageItem = memo(({ msg, handleSendMessage, handleDownload }: MessageItemProps) => {
  return (
    <div
...
export default function Home() {
  ...
  // ⚡ Bolt: Wrapped sendMessage with useCallback to provide a stable function reference
  // for MessageItem, ensuring memoization holds up across renders.
  ...
  // ⚡ Bolt: Wrapped handleSendMessage with useCallback for stable references to MessageItem
  const handleSendMessage = useCallback((text: string) => {
    if (!text.trim() || loading) return;
    sendMessage(text.trim());
  }, [loading, sendMessage]);

  // ⚡ Bolt: Wrapped handleDownload with useCallback for stable references to MessageItem
  const handleDownload = useCallback((url: string, source?: string) => {
    if (source === "Z-Library") {
