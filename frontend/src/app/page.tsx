...
export default function Home() {
  ...
  const handleDownload = useCallback(async (url: string, source?: string) => {
    ...
    if (source === "Anna's Archive") {
      ...
      setResolvingUrl(url);
      try {
        const res = await fetch(`${apiUrl}/api/annas-download?md5=${md5}`);
        if (!res.ok) throw new Error("resolve failed");
        const data = await res.json();
        if (win) {
          win.location.href = data.download_url;
        } else {
          openInNewTab(data.download_url);
        }
      } catch {
        // Fall back to the Anna's Archive page so the user can finish manually.
        if (win) {
    ...
    if (source === "Z-Library") {
      // Z-Library gates downloads behind its own page — open it directly.
      openInNewTab(url);
      return;
    }
