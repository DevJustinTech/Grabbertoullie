...
// ⚡ Bolt: Extracted MessageItem and wrapped in React.memo() to prevent unnecessary
// re-renders of the entire chat history on every keystroke in the input field.
const MessageItem = memo(({
  ...
  return (
    <div className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
      <div className={`max-w-[85%] sm:max-w-[75%] px-5 py-3.5 text-[15px] leading-relaxed flex flex-col gap-3 ${msg.role === "user" ? "bg-zinc-900 text-white rounded-3xl rounded-tr-sm" : "bg-white border border-zinc-200 text-zinc-800 rounded-3xl rounded-tl-sm shadow-sm"}`}>
...
export default function Home() {
  ...
  // ⚡ Bolt: Wrapped sendMessage, handleSendMessage, and handleDownload in useCallback
  // to provide stable function references to MessageItem, allowing React.memo to work properly.
