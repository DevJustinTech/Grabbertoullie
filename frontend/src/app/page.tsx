...
// ⚡ Bolt: Extract MessageItem and wrap with React.memo to prevent unnecessary re-renders when input changes
const MessageItem = memo(({ msg, onSendMessage, onDownload }: { msg: ChatMessage, onSendMessage: (text: string) => void, onDownload: (url: string, source?: string) => void }) => {
  return (
    <div
...
export default function Home() {
  ...
  // ⚡ Bolt: Wrap handleSendMessage in useCallback to maintain stable references and prevent MessageItem re-renders
  const handleSendMessage = useCallback((text: string) => {
    if (!text.trim() || loading) return;
    sendMessage(text.trim());
  }, [loading, sendMessage]);

  // ⚡ Bolt: Wrap handleDownload in useCallback to maintain stable references and prevent MessageItem re-renders
